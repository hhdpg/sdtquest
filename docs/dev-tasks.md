...
| 9 | Analyzer 模块 — 分类/汇总/日报 | ✅ | #2, #6 | 1-2天 | 已完成（含问题分类器/汇总器/报告生成器，39个测试通过） |
| 10 | API 层 — FastAPI 路由 + 启动入口 | ✅ | #7, #8 | 1天 | 已完成（含 FastAPI 应用/依赖注入/4个路由模块/完整启动入口） |
| 11 | 脚本开发 — 知识库构建/导入脚本 | ✅ | #4, #5, #6 | 1天 | 已完成（含代码解析/知识库构建/文档导入/分类器种子数据 4个脚本） |
| 12 | 测试 — 单元测试 + 集成测试 | 🔲 | #7 | 2天 | |
| 13 | 端到端联调 — 钉钉群实测 | 🔲 | #8,#10,#11,#12 | 2-3天 | |

**总进度**: 11/13 完成 · 0 进行中 · 0 被阻塞 · 2 可开始 · 0 取消  
**总计预估**: 15-20 个工作日
...
- ✅ 初始实现：规则 + 关键词匹配
  ...
  - 其他：不匹配以上规则
- ✅ 预留 ML 分类器接口（MLClassifier Protocol）
- ✅ 支持带置信度分类（classify_with_confidence）
- ✅ 批量分类功能（classify_batch）
...
- ✅ 活跃用户统计
- ✅ 趋势数据（按天统计）
- ✅ SummaryResult 和 CategoryStats 数据类
...
**测试**:
- ✅ 39 个单元测试全部通过
- ✅ 覆盖: classifier(12)、summarizer(14)、reporter(13)

**集成**:
- ✅ 已集成到 main.py，QuestionClassifier 自动注入 QAService

**交付清单**:
- `src/analyzer/classifier.py` — 问题分类器（规则+关键词匹配）
- `src/analyzer/summarizer.py` — 问题汇总器（统计分析）
- `src/analyzer/reporter.py` — 报告生成器（Markdown 日报）
- `src/analyzer/__init__.py` — 模块导出
- `tests/unit/test_analyzer/` — 39 个单元测试
...
- ✅ 预置分类样本数据（每类 30 条，共 120 条）
- ✅ 写入 SQLite 供分类器使用
- ✅ 支持清空、显示、统计等操作

**交付清单**:
- `scripts/parse_code.py` — 代码解析脚本（支持 JSON 输出）
- `scripts/build_knowledge.py` — 知识库构建脚本（支持增量/全量）
- `scripts/import_docs.py` — 文档导入脚本（支持文件/目录）
- `scripts/seed_classifier.py` — 分类器种子数据脚本（120条样本）

**验证结果**:
- ✅ 所有脚本 --help 正常显示
- ✅ seed_classifier.py 成功导入 120 条种子数据
- ✅ parse_code.py 成功解析测试项目（3页面/10按钮/2表单）
- ✅ 183 个单元测试全部通过（无回归）
